...
fn locks_the_codex_assistant_product_identity() {
    ...
    assert_eq!(tauri["version"].as_str(), Some("0.6.0"));
    assert_eq!(
    ...
    assert_eq!(package["version"].as_str(), Some("0.6.0"));
    assert_eq!(lockfile["name"].as_str(), Some("codex-assistant"));
    assert_eq!(lockfile["version"].as_str(), Some("0.6.0"));
    assert_eq!(
        lockfile["packages"][""]["name"].as_str(),
        Some("codex-assistant")
    );
    assert_eq!(lockfile["packages"][""]["version"].as_str(), Some("0.6.0"));
